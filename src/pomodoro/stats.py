...
def _empty() -> dict[str, object]:
    return {"total": 0, "today": 0, "last_date": _today()}


def load_stats() -> dict[str, object]:
    """Load stats from disk. Always returns a valid dict."""
    path = _stats_path()
    if not path.exists():
        return _empty()
    try:
